...
class Venta extends Conexion
{
 ...
 // Agregar producto o servicio al carrito (cajatmp)
    ...
    public function registrarVenta($idCliente, $metodoPago, $iva, $it) {
        ...
        $CarritoSql = $db->query("SELECT c.*, p.comision AS comision_unidad FROM cajatmp c LEFT JOIN producto p ON p.id = c.producto WHERE c.cliente = {$idCliente}");
        $subtotal = 0;
        $totalComision = 0;
        $items = [];
        ...
        while ($row = $CarritoSql->fetch_assoc()) {
            ...
            $comisionLinea = $row['comision_unidad'] * $row['cantidad'];
            $totalComision += $comisionLinea;
            $items[] = $row;
        }
        ...
        // Insertar factura
        $sqlFactura = "INSERT INTO factura (subtotal, iva, it, total, total_comision, fecha, hora, cliente, metodo_pago) 
                       VALUES ({$subtotal}, {$montoIva}, {$montoIt}, {$totalFactura}, {$totalComision}, NOW(), NOW(), {$idCliente}, '{$metodoPago}')";
        if ($db->query($sqlFactura)) {
            $idFactura = $db->insert_id;

            // Insertar detalles de venta
            foreach ($items as $it) {
                $db->query("INSERT INTO ventas (idfactura, producto, cantidad, precio, totalprecio, comision) 
                            VALUES ({$idFactura}, {$it['producto']}, {$it['cantidad']}, {$it['precio']}, {$it['totalprecio']}, {$it['comision']})");
            }

            // Limpiar carrito
            $db->query("DELETE FROM cajatmp WHERE cliente = {$idCliente}");
